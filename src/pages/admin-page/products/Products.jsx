...
import { getProduct, getProducts, searchedProducts, updateProduct } from '../../../redux/actions/product';
...
const Products = () => {
  ...
  const handleSearch = (e) => {
   ...
   const handleSubmit = (formData) => {
       ...
       dispatch(updateProduct({ editId: selectedProduct.id, formData })).unwrap().then(()=> {
  ...
  const filteredProduct = searched_products.length > 0 ? searched_products : products

  if (!error) {
    return (
      <>
        <ProdDelModal open={openDelModal} selectedProduct={selectedProduct} setSelectedProduct={setSelectedProduct} onCancel={() => setSelectedProduct(null)} />
          ...
          <div className=" flex sticky justify-between top-0 z-40 bg-white">
            ...
            <div className="flex-1 max-w-lg gap-4  flex items-center">
              <div className='flex flex-nowrap gap-2'>
                <button onClick={() => setView('list')} className={`${view === 'list' ? 'bg-primary' : 'bg-gray-300 '} rounded p-2`}>
                  <CiBoxList className={`${view === 'list' ? 'text-white' : 'text-gray-800 '} text-lg`} />

                </button>
                {' '}
                <button onClick={() => setView('grid')} className={`${view === 'grid' ? 'bg-primary' : 'bg-gray-300 '} rounded p-2`}>
                  <IoGridOutline className={`${view === 'grid' ? 'text-white' : 'text-gray-800 '} text-lg`} />

                </button>
              </div>

              <Search search={search} className="flex-1" setSearch={setSearch} handleSearch={handleSearch} />
            </div>
          </div>
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-right relative">
                          ...
                          {product.id === selectedRow
                            && (
                            <DropdownOptions toEdit={toEdit}  selectedRow={selectedRow} setSelectedRow={setSelectedRow} product={product} setSelectedProduct={setSelectedProduct} openDelModal={openDelModal} openUpdateModal={openUpdateModal} setOpenViewModal={setOpenViewModal} setOpenUpdateModal={setOpenUpdateModal} setSelectedRow={setSelectedRow} />
                            )}
        ...
        <UpdatePriceModal onUpdate={handleSubmit} open={openUpdateModal} product={selectedProduct} onClose={() => setOpenUpdateModal(false)} />
        <QuickProductViewModal open={openViewModal} onClose={()=> setOpenViewModal(false)} product={selectedProduct}/>
...
const DropdownOptions = ({
  selectedRow, setSelectedRow, setSelectedProduct, product,setOpenUpdateModal, setOpenViewModal
}) => {
    const navigate = useNavigate();

  return (
  <>
    <div onClick={() => setSelectedRow(null)} className="fixed  z-50 h-screen w-screen top-0 left-0 bg-gray-800/40 block ">                              </div>
    ...
    <div className="absolute top-0 right-full mr-2 bg-white p-2 border z-50 shadow-sm rounded-md min-w-[150px]">
      <ul className="text-left">
        <li onClick={() => {
          ...
          setSelectedRow(null)
          }} className="cursor-pointer hover:bg-gray-200 text-sm p-2">Views</li>
        <li onClick={() => {
          setSelectedProduct(product); 
          setOpenUpdateModal(true)

          setSelectedRow(null)
          }} className="cursor-pointer hover:bg-gray-200 text-sm p-2">Update Price</li>
        <li className="cursor-pointer hover:bg-gray-200 text-sm p-2" onClick={() => navigate(`/admin/edit/${product?.id}`)}>
          Edit
        </li>
      </ul>
    </div>
        ...
        <div className="grid gap-8 p-6 md:grid-cols-2">
          ...
          <div className="space-y-5">
            ...
            <div className="flex gap-3 pt-4">
              ...
              <button className="rounded-lg bg-primary px-5 py-3 text-sm font-medium text-white hover:opacity-90">
                Edit Product
              </button>
            </div>
          </div>
