...
const Apparels = ({ products, status, error }) => {
  if (products.length > 0) {
    const apparelsProducts = products.filter(
      (item) => item.product_category.name === "apparel",
    );
      ...
      return (
        <div>
          <header>
            ...
            <h4>
              {" "}
              You racquet catelogue is currently empty. add some products if you
              are an admin
            </h4>
          </header>
        </div>
  ...
  return (
    <h2 className="text-center">
      {" "}
      Please add some Racqet products if you are the admin
    </h2>
  );
};
