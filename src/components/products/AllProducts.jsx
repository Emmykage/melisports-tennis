...
const AllProducts = ({ products, status, error }) => {
  ...
  const racketProducts = products.filter(
    (item) => item.product_category.name === "racquet",
  );
  ...
  if (status === "success") {
    if (products.length < 1) {
      return (
        <div>
          <header>
            <h1 className="font-sans text-center text-3xl font-semibold">
              {" "}
    ...
    return (
      <>
        {products.map((product) => (
          <div
            key={product.id}
            className="products-display sm:min-w-48 w-full "
          >
            <div className="prod-img">
              <NavLink to={`/product-details/${product.id}`}>
                <img
                  src={
                    product.photo_urls ? product.photo_urls[0] : product.image
                  }
                  alt={product.name}
                />
              </NavLink>
